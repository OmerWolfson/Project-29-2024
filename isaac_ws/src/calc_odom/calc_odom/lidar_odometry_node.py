...
class LidarOdometryNode(Node):
    ...
    def scan_callback(self, msg):
        ...
        prev_ranges_array = np.array(self.prev_msg.ranges)
        prev_intensities_array = np.array(self.prev_msg.intensities)
        ...
        T, R1, t1 = Icp.best_fit_transform(self, prev_scan, curr_scan)
        # translation = np.dot(t1, R1)
        # delta_x = translation[0]
        # delta_y = translation[1]

        phi = -1.2047288
        normalize = 3.15587768
        delta_x = t1[0]*math.cos(phi)/ normalize
        delta_y = t1[1]*math.cos(phi)/ normalize
        r00 = R1[0, 0]
        if r00 < -1.0:
            r00 = -1.0
        elif r00 > 1.0:
