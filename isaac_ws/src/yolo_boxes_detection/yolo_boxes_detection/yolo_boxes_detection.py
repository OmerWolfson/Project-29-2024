...
# class YoloBoxDetection(Node):
#     def __init__(self):
#         super().__init__('yolo_box_detection')
#         self.declare_parameter('input_topic', '/g4_robot2/image_raw')
#         self.declare_parameter('output_topic', '/g4_robot2/image_yolo8')
#         self.input_topic = self.get_parameter('input_topic').get_parameter_value().string_value
#         self.output_topic = self.get_parameter('output_topic').get_parameter_value().string_value
        
#         self.bridge = CvBridge()
#         self.model = YOLO('runs/detect/train3/weights/best.pt')  # Use the trained YOLO model
...
class YoloBoxDetection(Node):
    def __init__(self):
        ...
        self.declare_parameter('output_topic', '/g4_robot1/image_yolo8')
        self.input_topic = self.get_parameter('input_topic').get_parameter_value().string_value
        self.output_topic = self.get_parameter('output_topic').get_parameter_value().string_value
    ...
    def listener_callback(self, msg):
        try:
            ...
            for result in results:
                for det in result.boxes:
                    conf = det.conf.item() if hasattr(det.conf, 'item') else det.conf  # Ensure conf is a number
                    if det.cls == 0 and conf >= 0.5:  # Filter by class and confidence threshold
                        xyxy = det.xyxy[0].tolist()
                        label = f'Box {conf:.2f}'  # Format confidence value
                        detected_objects.append(label)
                        # Draw bounding box and label on image
